...
<body>
  <header>
    <div class="site-title">ナナ 設定資料</div>
    <nav>{nav}
      <a href="mobile.html">スマホ版</a>
...
def mobile_template(body: str, toc: str) -> str:
    return f"""<!doctype html>
<html lang="ja">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>ナナ 設定資料 スマホ版</title>
  <style>
    :root {{
      color-scheme: light;
      --bg: #f7f3ea;
    ...
    * {{ box-sizing: border-box; }}
    ...
    section {{
      ...
      padding: 18px 16px;
      margin: 0 0 14px;
      box-shadow: 0 1px 6px rgba(36, 33, 29, 0.04);
    }}
    h1, h2, h3 {{
      line-height: 1.35;
      letter-spacing: 0;
...
<body id="top">
  <header>
    <div class="site-title">ナナ 設定資料 スマホ版</div>
    <nav class="toc">{toc}</nav>
  </header>
  <main>
    <div class="mobile-intro">
      <h1>ナナ 設定資料</h1>
      <p>スマホで読むために、設定ノート・全体構成・世界設定・キャラクター・10年旅・英雄パーティ・主題を1ページにまとめた版です。</p>
    </div>
    {body}
  </main>
</body>
...
def mobile_page(files: list[Path], titles: dict[str, str]) -> str:
    toc_items = []
    sections = []
    for path in files:
        section_id = path.stem
        title = titles[path.name]
        toc_items.append(f'<a href="#{html.escape(section_id)}">{html.escape(title)}</a>')
        markdown = strip_first_h1(path.read_text(encoding="utf-8"))
        body = markdown_to_html(markdown)
        sections.append(
            f'<section id="{html.escape(section_id)}">\n'
            f'<h2>{html.escape(title)}</h2>\n'
            f'{body}\n'
            f'<a class="top-link" href="#top">目次へ戻る</a>\n'
            f'</section>'
        )
...
def main() -> None:
    ...
    (SITE / "mobile.html").write_text(mobile_page(files, titles), encoding="utf-8")
    (SITE / "style.css").write_text(STYLE, encoding="utf-8")
    print(f"Generated {len(files)} pages, mobile page, and editor in {SITE}")
...
`;
    ...
    function saveLocalConfig(showStatus = true) {{
      const c = config();
      localStorage.setItem('nanaEditorConfig', JSON.stringify({{
        owner: c.owner,
        repo: c.repo,
        branch: c.branch,
        basePath: c.basePath
      }}));
      sessionStorage.setItem('nanaEditorToken', c.token);
      if (showStatus) setStatus('設定を保存しました。tokenはこのブラウザのセッション内だけに保存します。');
    }}

    function loadLocalConfig() {{
      try {{
        const saved = JSON.parse(localStorage.getItem('nanaEditorConfig') || '{{}}');
        els.owner.value = saved.owner || '';
        els.repo.value = saved.repo || '';
        els.branch.value = saved.branch || 'main';
        els.basePath.value = saved.basePath || 'ナナ';
        els.token.value = sessionStorage.getItem('nanaEditorToken') || '';
      }} catch (_) {{
        els.branch.value = 'main';
        els.basePath.value = 'ナナ';
      }}
    }}
